...
func main() {
	...
	for i := range[2010..2014] {
		content, err := ioutil.ReadFile("schoolEthnicity" + i + ".csv")
		lines := strings.Split(string(content), "\r")
		//populate schools..
		rows,err := db.Query("CREATE TABLE IF NOT EXISTS schools ( id INT(11) NOT NULL AUTO_INCREMENT, name varchar(255) NOT NULL NULL, PRIMARY KEY (id));")
		check(err)
			...
			if i > 1 {
				...
				if splitStr[3] != "" {
					...
					if !hasNextRow {
						...
						// fmt.Println(splitStr[startingIndex])
						// fmt.Println(splitStr[startingIndex + 1])
